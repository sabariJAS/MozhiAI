\# 🚀 Mozhi AI



\## Real-Time Tamil → English AI Voice Translator



\*\*Speak Tamil → Translate with AI → Generate English Voice → Virtual Microphone\*\*



Mozhi AI is an AI-powered voice translation application that converts spoken Tamil into natural English voice output.



\### 🌐 Official Website



👉 \[Visit the Official Mozhi AI Website](https://mozhiai-seven.vercel.app/)



Explore the project, view the application showcase, and download the Windows version.



\---



\## 📚 Project Documentation



All detailed documentation, code explanations, project details, technical information, setup instructions, and development information are available in the Project Wiki.



👉 \[Go Directly to the Mozhi AI Wiki](https://github.com/sabariJAS/MozhiAI/wiki)



\---



\## 🔄 How It Works



\*\*Tamil Speech → Speech-to-Text → AI Translation → English Voice Generation → Virtual Microphone\*\*



\---



\## 🛠️ Technology



\* \*\*Python\*\* — Core application and processing

\* \*\*Deepgram\*\* — Speech-to-text and voice generation

\* \*\*Groq\*\* — AI-powered translation

\* \*\*SoundDevice\*\* — Audio recording and device routing

\* \*\*SoundFile\*\* — Audio file processing

\* \*\*NumPy\*\* — Audio data processing

\* \*\*SciPy\*\* — WAV and audio processing



\---



\## 💻 Platform



\*\*Windows 10 or later\*\*



The application requires a microphone, internet connection, API access, and a compatible virtual audio device for virtual microphone output.



\---



\## 📌 Project Status



\*\*Active Development\*\*



Mozhi AI is currently under development. Features, supported platforms, AI models, and integrations may change over time.



\---



\## 📖 Documentation



For complete project information:



👉 \[Open Mozhi AI Wiki](https://github.com/sabariJAS/MozhiAI/wiki)



\---



\*\*Mozhi AI\*\*

\*Tamil Speech → AI Translation → English Voice → Virtual Microphone\*



